...
import api from "../api"; // ✅ Usamos tu instancia de axios
...
const Analiticas = () => {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const { data: rawData } = await api.get("/api/pedidos/analiticas/"); // ✅ Ahora dinámico

        if (rawData) {
          const formattedData = [
            {
              estado: "en_proceso",
              porcentaje: rawData.en_proceso.porcentaje,
              count: rawData.en_proceso.count,
            },
            {
              estado: "empaquetado",
              porcentaje: rawData.empaquetado.porcentaje,
              count: rawData.empaquetado.count,
            },
            {
              estado: "entregado",
              porcentaje: rawData.entregado.porcentaje,
              count: rawData.entregado.count,
            },
            {
              estado: "cancelado",
              porcentaje: rawData.cancelado.porcentaje,
              count: rawData.cancelado.count,
            },
          ];

          // ✅ Filtrar estados con porcentaje mayor a 0
          const filteredData = formattedData.filter(
            (item) => item.porcentaje > 0
          );
